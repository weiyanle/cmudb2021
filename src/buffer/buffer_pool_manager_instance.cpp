...
Page *BufferPoolManagerInstance::NewPgImp(page_id_t *page_id) {
  ...
  if (!(free_list_.empty() && replacer_->Size() == 0)) {
    ...
    if (!free_list_.empty()) {
      ...
      if (!replacer_->Victim(&vic_p)) {  // if all the pages are pinned
        latch_.unlock();
        return nullptr;
      }
...
Page *BufferPoolManagerInstance::FetchPgImp(page_id_t page_id) {
  ...
  if (page_table_.find(page_id) != page_table_.end()) {
    ...
    if (pages_[page_table_[page_id]].pin_count_ < 0) {
      LOG_DEBUG("Page %d Pin count = %d before fetching", page_id, pages_[page_table_[page_id]].pin_count_);
    }
    ...
    for (frame_id_t i = 0; i < static_cast<int>(pool_size_); ++i) {
      LOG_INFO("frame %d page %d pin_count %d", i, pages_[i].GetPageId(), pages_[i].GetPinCount());
    }
  }
  latch_.unlock();
  return ret;
...
bool BufferPoolManagerInstance::UnpinPgImp(page_id_t page_id, bool is_dirty) {
  ...
  if (page_table_.find(page_id) != page_table_.end()) {
    if (pages_[page_table_[page_id]].pin_count_ <= 0) {
      LOG_DEBUG("Page %d Pin count = %d before unpinning", page_id, pages_[page_table_[page_id]].pin_count_);
    }
    ret = true;
